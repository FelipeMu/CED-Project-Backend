...
const getEventosNiveles= async (req,res) => { 
    const response = await pool.query('SELECT DISTINCT pr.id as idevento, pr.nombre as name, pr.hora_inicio as start, pr.hora_termino as end, pr.detalles as details, (SELECT md.tipo_modalidad as modalidad FROM modalidad as md WHERE md.codigo = pr.id_modalidad), (SELECT te.tipo as tipo_evaluacion FROM tipo_evaluacion as te WHERE te.id = pr.id_tipo_evaluacion), pr.color FROM (SELECT DISTINCT pr.id as id_pr_otros, pr.id_profesor_dicta FROM (SELECT DISTINCT pl.nivel + 1 as nivelUp, pl.nivel -1 as nivelDown, pd.id_profesor as id_prof FROM profesor_dicta as pd, plan_nivel as pl WHERE pd.id_profesor = $1 and pd.codigo_asignatura = pl.codigo_asignatura) as prof, plan_nivel as pl, programar as pr, profesor_dicta as pd WHERE pr.id_admin = 2 and ((pl.nivel = prof.nivelUp or pl.nivel = prof.nivelDown) and pr.codigo_asignatura = pl.codigo_asignatura) and pd.id = pr.id_profesor_dicta and pd.id_profesor != prof.id_prof) as ids, programar as pr WHERE pr.id = ids.id_pr_otros', [req.params.id_profesor]);
    res.json(response.rows);
};
///////////////////////////////////////////////////////////
